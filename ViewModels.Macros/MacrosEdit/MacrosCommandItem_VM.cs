...
public class MacrosCommandItem_VM : ReactiveObject
{
    ...
    public MacrosCommandItem_VM(Guid id, EditCommandParameters parameters, Action<Guid> runCommandHandler, Action<Guid> editCommandHandler, Action<Guid> removeItemHandler, IMessageBox messageBox)
    {
        ...
        Command_RunCommand.ThrownExceptions.Subscribe(error => messageBox.Show(LocalizationProvider.Get("Message.Error.RunCommand", CommandName ?? string.Empty) + "\n\n" + error.Message, MessageType.Error, error));

        Command_EditCommand = ReactiveCommand.Create(() => editCommandHandler(Id));
        Command_EditCommand.ThrownExceptions.Subscribe(error => messageBox.Show(LocalizationProvider.Get("Message.Error.EditCommand", CommandName ?? string.Empty) + "\n\n" + error.Message, MessageType.Error, error));

        Command_RemoveCommand = ReactiveCommand.CreateFromTask(async () =>
        {
            if (await messageBox.ShowYesNoDialog(LocalizationProvider.Get("Message.Confirm.DeleteCommand", CommandName ?? string.Empty), MessageType.Warning) == MessageBoxResult.Yes)
            {
                removeItemHandler(Id);
            }
        });
        Command_RemoveCommand.ThrownExceptions.Subscribe(error => messageBox.Show(LocalizationProvider.Get("Message.Error.RemoveCommand", CommandName ?? string.Empty) + "\n\n" + error.Message, MessageType.Error, error));
    }
}
